...
#
# Config parsing has no external dependencies (no yq needed) — pure awk
...
if [ ! -f "$CONFIG_FILE" ]; then
    ...
    echo "       Create it with:  cp config.example.yaml config.yaml" >&2
...
# Print "name-ip" pairs for a list section (masters or workers).
cfg_nodes() {
    awk -v section="$1" '
        $0 ~ "^" section ":" { in_s = 1; next }
        /^[^[:space:]]/ { in_s = 0 }
